    ...
    def tokenize(self, text, ptb=False):
        ...
        for i, word in enumerate(words):
            ...
            if m:
                pre = m.group(1)
                if (
                    (re.search("\.", pre) and re.search("[a-zA-Z]", pre))
                    or (pre in self.nonbreaking_prefixes[self.language])
                    or ((i < len(words) - 1) and re.match("^\d+", words[i + 1]))
                ):
                    pass  # do nothing
                elif (
                    (pre in self.nonbreaking_prefixes_numeric[self.language])
                    and (i < len(words) - 1)
                    and re.match("\d+", words[i + 1])
                ):
                    pass  # do nothing
                else:
                    word = pre + " ."
