...
def batch_multi_pre_rec_f1_row(
    candidate,
    source,
    golds_set,
    max_unchanged_words,
    ...
    for annotator, gold in golds_set.items():
        ...
        f1_local = comp_f1(
            stat_correct_local, stat_proposed_local, stat_gold_local, beta
        )

        if (
            f1_max < f1_local
            or (f1_max == f1_local and max_stat_correct < stat_correct_local)
            or (
                f1_max == f1_local
                and max_stat_correct == stat_correct_local
                and min_stat_proposed + sqbeta * min_stat_gold
...
# precision, recall, F1
def precision(
    candidate, source, gold_edits, max_unchanged_words=2, beta=0.5, verbose=False
):
    return pre_rec_f1(
        candidate, source, gold_edits, max_unchanged_words, beta, verbose
    )[0]
    ...
    for e in reversed(editSeq):
        for i in range(last_index, len(goldSeq)):
            ...
            if matchEdit(e, g, ignore_whitespace_casing):
                ...
                if verbose:
                    nextEditList = [
                        shrinkEdit(edit) for edit in editSeq if e[1] == edit[0]
                    ]
                    prevEditList = [
                        shrinkEdit(edit) for edit in editSeq if e[0] == edit[1]
                    ]

                    if e[0] != e[1]:
                        nextEditList = [
                            edit for edit in nextEditList if edit[0] == edit[1]
                        ]
                        prevEditList = [
                            edit for edit in prevEditList if edit[0] == edit[1]
                        ]
                    else:
                        nextEditList = [
                            edit
                            for edit in nextEditList
                            if edit[0] < edit[1] and edit[3] == ""
                        ]
                        prevEditList = [
                            edit
                            for edit in prevEditList
                            if edit[0] < edit[1] and edit[3] == ""
                        ]

                    matchAdj = any(
                        any(
                            matchEdit(edit, gold, ignore_whitespace_casing)
                            for gold in goldSeq
                        )
                        for edit in nextEditList
                    ) or any(
                    ...
                    if (
                        e[0] < e[1]
                        and len(e[3].strip()) == 0
                        and (len(nextEditList) > 0 or len(prevEditList) > 0)
                    ):
                        if matchAdj:
                            print("!", e)
                        else:
